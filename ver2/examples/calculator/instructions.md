# Построение калькулятора с помощью DSL1 (ver2_1c)

## Пошаговое руководство

*Аналог «Hello, Calculator» из BPMN Runa WFE.*

---

## Содержание

0. [Схема процесса (укрупнённая)](#0-схема-процесса-укрупнённая)
...
## 0. Схема процесса (укрупнённая)

Калькулятор как «чёрный ящик»: вход, обработка, выход.
...
│                                     │
│  УПРАВЛЕНИЕ                         │
│  [Вычислить] [Очистить] [Выход]     │
└─────────────────────────────────────┘
```

Жизненный цикл (укрупнённо):
...
[Открытие страницы]
        ...
        ▼
[ИНИЦИАЛИЗАЦИЯ]
  loadOntology() → N3.Store
  initDropdown() → заполнение #Operation
        │
        ▼
[Интерфейс готов]
        │
        ├──► Пользователь вводит Input1, Input2, выбирает Operation
        │
        ├──► [Вычислить] → calculate() → Result / status
        │
        ├──► [Очистить]  → handleClear() → очистка полей
        │
        └──► [Выход]     → EndEvent (window.close)
```
